...
findK k l = head (drop k l) -- error when k >= length l
-- also
findK2 k l = l !! k
...
-}
duplicate2 xs = concat [[x, x] | x <- xs] -- this is from gh copilot

duplicate xs = if null xs then [] else [head xs, head xs] ++ duplicate (tail xs)


duplicate3 [] = []
duplicate3 (x : xs) = [x, x] ++ duplicate3 xs
...
splitAtIndex2 k l = splitAt k l   -- ok

-- Drop the element at index k in list l
-- For example "dropK 3 [0,0,0,1,0,0,0]" returns [0,0,0,0,0,0]
helper (x, y) = x ++ tail y
dropK k l = helper (splitAtIndex k l)

dropK2 k l = let (x, y) = splitAtIndex k l in x ++ tail y
...
slice i k l = let (x, _) = splitAtIndex k l in let (_, y) = splitAtIndex i x in y

-- Insert element x in list l at index k
-- For example, "insertElem 2 5 [0,0,0,0,0,0]" returns [0,0,0,0,0,2,0]
insertElem y k l = let (x, z) = splitAtIndex k l in x ++ [y] ++ z
...
rotate2 0 l = l
rotate2 n l = rotate2 (n - 1) (tail l ++ [head l])
